...
func is_point_in_collision_area(point: Vector2) -> bool:
	var nav_region = get_node("/root/BattleView/Map/NavRegion")
...
func get_latest_containing_rect_for_faction(i_faction : int) -> Rect2:
	var root_node = get_node("/root/BattleView/")
	var unit_factory = root_node.get_unit_factory()
